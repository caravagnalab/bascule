...
refine_signatures_SBS <- function(x, threshold=0.8) {
...
refine_signatures_DBS <- function(x, threshold=0.8) {
  
  exposure <- basilica:::long_to_wide(x$nmf$DBS$exposure, what = "exposures")
  fixed <- basilica:::long_to_wide(x$nmf$DBS$beta_fixed, what = "beta")
  denovo <- basilica:::long_to_wide(x$nmf$DBS$beta_denovo, what = "beta")
  a <- basilica:::cosine.matrix(fixed, denovo)
  while (max(a) >= threshold) {
    row <- which(a == max(a), arr.ind = T)[1]
    col <- which(a == max(a), arr.ind = T)[2]
  ...
  x$nmf$DBS_refined$exposure <- basilica:::wide_to_long(exposure, what = "exposures")
  x$nmf$DBS_refined$beta_fixed <- x$nmf$DBS$beta_fixed
  x$nmf$DBS_refined$beta_denovo <- basilica:::wide_to_long(denovo, what = "beta")
