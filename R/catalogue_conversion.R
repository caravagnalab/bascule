...
"get.SBS.counts" <- function( data, reference = NULL ) {
  ...
  data <- GRanges(as.character(data$chrom),
                  IRanges(start=(as.numeric(data$pos)-1),width=3),
                  ref=DNAStringSet(as.character(data$ref)),
                  alt=DNAStringSet(as.character(data$alt)),
  ...
  data$cat <- ifelse(data$ref %in% c("C","T"),
...
#'
#' @examples
conversion <- function(counts, reference_genome) {
  counts  <- counts %>%
    dplyr::mutate(
      CHROMOSOME = chr,
      START = from,
  ...
  # remove 'chr' from CHROMOSOME column entries
  ...
  trinucleotides_counts = get.SBS.counts(data=unique(counts), reference=reference_genome)
