...
library(basilica)
basilica::configure_environment()
```


We can load the data `example_dataeset`, a basilica object containing the true signatures and exposures used to generate the mutation counts matrix. In the object, data for SBS and DBS is reported.
```{r}
