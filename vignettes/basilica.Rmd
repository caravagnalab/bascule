...
You can install basilica from GitHub using `devtools`.
...
- if there is no `conda` environment loaded, the package will check if the `basilica-env` is present, otherwise it will be created,

- to use an existing `conda` environment, it can be loaded **before** loading the package, either through the `reticulate` function `reticulate::use_condaenv()` or by using the `basilica` function `load_conda_env()`:
...
configure_environment(env_name="basilica-env", use_default=F)
```
